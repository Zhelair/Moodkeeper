...
(function(){
  ...
  async function getWeekEntries(baseDate){
  ...
  TrackboardRouter.register('goals', async (mount)=>{
    TrackboardUI.setActiveNav('goals');
    TrackboardUI.setSubtitle(`Goals · ${TrackboardUI.fmtActive ? TrackboardUI.fmtActive() : TrackboardUI.activeISO()} · Private · Stored on this device`);

    const baseDate = new Date((TrackboardUI.activeISO ? TrackboardUI.activeISO() : TrackboardUI.todayISO()) + 'T00:00:00');
    const wk = isoWeekKey(baseDate);
    const weekRec = await Store.getWeek(wk) || {weekKey:wk};
    const { start, entries } = await getWeekEntries(baseDate);
