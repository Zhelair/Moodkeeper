...
(function(){
  ...
  function render(view){
    ...
    async function setTodayValue(id, val){
      const today = TrackboardUI.activeISO ? TrackboardUI.activeISO() : TrackboardUI.todayISO();
    ...
    async function renderTab(){
      ...
      $$(".practice-controls").forEach(ctrl=>{
        ...
        ctrl.addEventListener('click', async (e)=>{
          ...
          if(!act) return;
          if(item.kind==='bool'){
            const todayV = !!getPracticeValue(week.map[(TrackboardUI.activeISO ? TrackboardUI.activeISO() : TrackboardUI.todayISO())]||{}, id);
  ...
  TrackboardRouter.register('practices', (view)=>{
    if(window.TrackboardUI){
      TrackboardUI.setActiveNav('practices');
      TrackboardUI.setSubtitle(`Practices · ${TrackboardUI.fmtActive ? TrackboardUI.fmtActive() : TrackboardUI.activeISO()} · Private · Stored on this device`);
    }
    render(view);
  });
})();
